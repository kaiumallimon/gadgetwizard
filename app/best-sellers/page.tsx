...
function buildBestSellersHref(input: {
  page: number;
  pageSize: number;
  search?: string;
  minPrice?: number;
  ...
  return queryString ? `/best-sellers?${queryString}` : "/best-sellers";
}

export default async function BestSellersPage(context: {
  ...
  const facets = await getPublicProductFilterFacets({ search, bestSellersOnly: true });
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">
              Showing {rangeStart}-{rangeEnd} of {result.total} best seller products
            </p>
          </div>
        </div>
      </header>
        ...
        <div className="space-y-5">
          <div className="rounded-2xl border border-zinc-200/80 bg-white/90 p-4 shadow-sm backdrop-blur">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <Badge variant="outline">Best sellers only</Badge>
          ...
          {result.items.length === 0 ? (
            <section className="rounded-2xl border border-zinc-200 bg-white p-10 text-center text-zinc-500">
              No best seller products match your current filters.
            </section>
          ) : (
