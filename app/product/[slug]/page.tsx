  ...
  return (
    <div className="mx-auto grid w-full max-w-6xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[1.1fr_1fr]">
      <div className="space-y-3">
        <div className="relative h-72 overflow-hidden rounded-3xl border border-white/10 bg-white/5 sm:h-[420px]">
          <img
      ...
      <section className="space-y-4 rounded-3xl border border-white/10 bg-white/5 p-6">
        <p className="text-sm uppercase tracking-[0.2em] text-[color:var(--muted)]">{product.categoryName}</p>
        <h1 className="text-3xl font-semibold text-white">{product.name}</h1>
        <p className="text-[color:var(--muted)]">{product.description ?? "No description provided."}</p>
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-[color:var(--muted)]">Stock: {product.stock}</p>
        </div>
