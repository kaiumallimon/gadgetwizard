...
function buildBrandHref(input: {
  ...
  return queryString ? `/brand/${input.slug}?${queryString}` : `/brand/${input.slug}`;
}

export default async function BrandPage(context: {
  ...
  const brandNameBySlug = new Map(facets.brands.map((entry) => [entry.slug, entry.name]));
    ...
    <div className="mx-auto w-full max-w-7xl space-y-6 px-4 py-8 sm:px-6">
      <header className="space-y-3 rounded-2xl border border-zinc-200/80 bg-white/90 p-5 shadow-sm backdrop-blur sm:p-6">
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight text-zinc-900">Brand: {brand.name}</h1>
          ...
          <div className="rounded-2xl border border-zinc-200/80 bg-white/90 p-4 shadow-sm backdrop-blur">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              {selectedBrandSlugs.map((brandSlug) => (
                <Badge key={brandSlug} variant="outline">Brand: {brandNameBySlug.get(brandSlug) ?? brandSlug}</Badge>
              ))}
